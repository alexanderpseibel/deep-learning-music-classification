...
def init_wandb(config: dict, project_name: str = "nlp-mini-project"):
    import os
    # Extend W&B backend timeouts to avoid ReadTimeout on slow clusters
...
def log_metrics(metrics: dict):
    try:
        ...
        print(f"[WARN] W&B metric log failed: {e}")
...
def log_confusion_heatmap(y_true, y_pred, class_names):
    ...
    sns.heatmap(M, cmap="viridis", xticklabels=class_names, yticklabels=class_names, ax=ax)
    plt.tight_layout()
...
def log_error_heatmap(y_true, y_pred, class_names):
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.heatmap(M, cmap="magma", xticklabels=class_names, yticklabels=class_names, ax=ax)
    plt.tight_layout()
...
def log_precision_recall(y_true, y_probs, class_names):
    ...
    for i in range(num_classes):
        ...
        # Skip classes with very dense PR curves to limit W&B upload size
        if len(prec) > 2000:
            print(f"[WARN] PR curve for {class_names[i]} skipped ({len(prec)} points)")
            continue
        try:
            ...
            print(f"[WARN] W&B PR logging failed for {class_names[i]}: {e}")
